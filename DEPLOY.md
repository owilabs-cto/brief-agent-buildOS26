...
1. **AzDO ↔ GitHub service connection.** In OWI's Azure DevOps project, add a GitHub service connection pointing at `owilabs-cto/brief-agent-buildOS26`. The existing `aks-reelcruit-service-connection` (Kubernetes) and `dockerhub-reelcruit` (Docker registry) connections can be reused as-is.
...
3. **Copy + rename the k8s manifests.** Start from `audit-agent/k8s/orchestrator/{base,overlays/production}/`. Global rename `owi-insight-orchestrator → brief-agent`, change `containerPort: 8080 → 3030`, update probe paths from `/api/health → /health`, and trim `overlays/production/patches/env.yaml` to only the env vars this binary actually reads. Target namespace stays `public-ingress` (same as audit-agent).
4. **Registry.** Push to Docker Hub `reelcruitapp/brief-agent` — same account as `reelcruitapp/owi-insight-orchestrator`. Not ACR (OWI's stack does not use ACR despite the Azure-everywhere naming).
...
- **No `azure-pipelines.yml`** — committing a pipeline that references `dockerhub-reelcruit` and `aks-reelcruit-service-connection` without those connections being wired to this GitHub repo would silently fail every push to main. The migration is short enough that pre-writing the YAML buys nothing.
